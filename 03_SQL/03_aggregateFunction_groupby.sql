...
select count(distinct dept_name) from emp; -- 부서가 몇개 : 11
select count(distinct ifnull(dept_name, 'a'))from emp; -- 결측치 포함한 개수 : 12
...
-- select	emp_name, max(char_length(emp_name))-> 젤큰거 하나 나왔는데 이름은 107개니까 표를 못 만듦. 집계한 컬럼에 그냥 컬럼은 같이 못함.
...
-- 입사연도 별 직원들의 급여 평균.
select	year(hire_date), avg(salary) from emp
group by year(hire_date)
order by 1; -- 함수가 처리한 결과가 같은 행끼리 group으로 묶는다.
...
select	if(salary < 10000, '만 미만', '만 이상') "니 돈좀 버나 ㅋ",
		count(*) "직원 수 ㅋ"
from	emp
...
from	emp
group by dept_name
having avg(salary) >= 5000 and count(*) >= 5; -- salary 평균은 5000이 넘고 직원이 5명 이상만.
-- 그룹 나누고(group by) 집계(having)한거임
...
from	emp
group by dept_name
having avg(salary) >= 5000 and sum(salary) >= 5000;
