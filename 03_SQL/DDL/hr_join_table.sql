...
CREATE TABLE emp(
    ...
    dept_id   INT, -- 참조 table에 의 PK
    CONSTRAINT fk_emp_dept FOREIGN KEY(dept_id) REFERENCES dept(dept_id) ON DELETE SET NULL, -- 참조하고 있는 pk 값을 지우면 fk의 값들이 null로 바뀜
    CONSTRAINT fk_emp_job FOREIGN KEY(job_id) REFERENCES job(job_id) ON DELETE SET NULL, --  PK값과 
