...
@st.?  # cache에 올리겠다는 뜻. cache에 올라가 있으니 rerun해도 이 함수를 실행하지 않음. cache에 새로운 내용 올릴려면 streamlit을 껐다켜야함.
def get_llm_model():
    ?   # .env 파일 로드 (API 키 등)
    # OpenAI 모델 인스턴스 생성
    ...
    messages = [
        ...
        ?(variable_name = "history", optional = True),    # 대화 이력
        ("user", "{query}")    # 사용자 입력
        ]
    )
...
def get_chain():
    ...
    chain = ?(
        ...
        input_messages_key= "?",    # 사용자 입력 키
        history_messages_key= "?"   # 히스토리 키
    )
    return chain
...
if "messages" not in st.?:   # return T/F
    st.?["messages"] = []    # 대화 내용들을 저장할 list를 "messages" 키로 저장


# session_state에서 session_id를 조회, 없으면 빈 상태값을 저장.
if "session_id" not in st.?:
    st.?["session_id"] = None
...
for message in message_list:
    with st.chat_message(message["role"]):
        st.?(message["content"])    # 출력


# user의 prompt를 입력 받는 위젯
prompt = st.?("User Prompt")    # user가 입력한 문자열을 반환.
