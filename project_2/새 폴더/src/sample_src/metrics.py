...
def plot_regression_result(y_true, y_pred, title="예측 vs 실제 산점도"):
    # plt.rcParams['font.family'] = 'Malgun Gothic'
    plt.figure(figsize=(7, 5))
    # sns.scatterplot(x=y_true, y=y_pred, color='steelblue', edgecolor='black')
    plt.plot([y_true.min(), y_true.max()], [y_true.min(), y_true.max()], 'r--', label='완벽 예측선')
    plt.xlabel("실제 은퇴 나이")
    plt.ylabel("예측 은퇴 나이")
    plt.title(title)
    plt.legend()
