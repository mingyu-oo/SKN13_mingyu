...
# LLM model 생성
@st.cache_resource  # cache에 올리겠다는 뜻. rerun하면 cache에 올라가 있으니 이 함수를 실행하지 않음. 다시 껐다 켜야함
def get_llm_model():
    load_dotenv()
...
if prompt is not None:
    ...
    st.session_state["messages"].append({"role" : "AI", "content" : ai_message.content})
    print(type(st.session_state),st.session_state)
