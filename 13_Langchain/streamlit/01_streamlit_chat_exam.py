...
prompt = st.chat_input()    # user가 입력한 문자열을 반환.
print(prompt)

## 대화 작업
if prompt is not None:
    # LLM에 prompt 전달 -> 응답 -> 화면에 대화내용을 출력(chat_message)
    with st.chat_message("User"):   # Container에 출력할 메세지가 누구의 것인지 지정하면서 Container 조회.
        st.write(prompt)    # with에서 조회한 Container에 출력.
